...
-- PLAY BOOK STEP.  For any COMMON LOL processes determine if they created any files   
...
WHERE spj.eventtype = 0 AND spj.time > strftime('%s','now','-7 days') 
   AND (spj.processname IN ('powershell.exe', 'cmd.exe', 'netsh.exe', 'rundll32.exe','explorer.exe') OR spj.processname LIKE 'python%.exe')
   AND u.username NOT IN('SYSTEM', 'LOCAL SERVICE', '')